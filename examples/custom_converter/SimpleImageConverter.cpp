...
// to infer the "string" name of the data-type
...
string SimpleImageConverter::serialize(const AnnotatedData &data, string &wire) {
    ...
    copy((char*) &image->width, ((char*) &image->width) + 4,
	 wire.begin());
    copy((char*) &image->height, ((char*) &image->height) + 4,
	 wire.begin() + 4);
...
AnnotatedData SimpleImageConverter::deserialize(const string &wireSchema, const string &wire) {
